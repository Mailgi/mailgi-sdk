...
export function registerMailCommands(program) {
    ...
    program
        ...
        table(['ID', 'From', 'Subject', 'Date', ''], result.messages.map((m) => [
            m.id,
            m.from[0]?.email ?? '',
    ...
    program
        ...
        .requiredOption('--agent <id>', 'agent email or username')
        .action(async function (msgId, opts) {
        const { json } = this.optsWithGlobals();
        const { client } = getClient(opts.agent);
        let msg;
        try {
        ...
        if (msg.cc?.length)
            ok(`CC:      ${msg.cc.map((a) => a.email).join(', ')}`);
    ...
    program
        ...
        const { client } = getClient(opts.agent);
        const split = (s) => s ? s.split(',').map((x) => x.trim()) : undefined;
        let result;
        try {
    ...
    program
        ...
        if (json) {
            printJson({ deleted: msgId });
            return;
        }
        ok(`Moved to Trash: ${msgId}`);
    });
}
function resolveBody(body, bodyValues) {
    if (!body)
        return null;
